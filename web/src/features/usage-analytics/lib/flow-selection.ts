...
/**
 * Stable identity for a node filter. The NUL separator keeps a {kind, id} pair
 * from colliding with a different split of the same characters — both the
 * chart and the filter popover key their selections with this.
 */
export function flowNodeFilterKey(filter: FlowNodeFilter): string {
