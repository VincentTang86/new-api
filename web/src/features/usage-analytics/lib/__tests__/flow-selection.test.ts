...
describe('flowNodeFilterKey', () => {
  test('keeps a kind/id pair from colliding with a different split', () => {
    // Both the Sankey and the filter popover key their selections with this,
    // so a collision would silently merge two unrelated chips.
    expect(flowNodeFilterKey({ kind: 'user', id: 'a' })).not.toBe(
      flowNodeFilterKey({ kind: 'user', id: '' })
    )
    expect(flowNodeFilterKey({ kind: 'user', id: '1' })).not.toBe(
      flowNodeFilterKey({ kind: 'token', id: '1' })
    )
  })

  test('is stable for the same pair', () => {
    expect(flowNodeFilterKey({ kind: 'model', id: 'gpt-4' })).toBe(
      flowNodeFilterKey({ kind: 'model', id: 'gpt-4' })
    )
  })
})
