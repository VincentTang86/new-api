...
import { useModelStatCardsConfig } from '@/features/admin-dashboard/hooks/use-dashboard-config'
import { calculateDashboardStats } from '@/features/admin-dashboard/lib'
...
interface LogStatCardsProps {
  ...
  loading: boolean
  error: boolean
  // Window span in minutes, used for the average RPM/TPM cards.
  rangeMinutes: number
}
...
export function LogStatCards(props: LogStatCardsProps) {
  ...
  const stats = useMemo(() => calculateDashboardStats(props.data), [props.data])

  const adaptedStats = {
    rpm: stats.totalCount,
    quota: stats.totalQuota,
    tpm: stats.totalTokens,
  }

  const items = statCardsConfig.map((config) => {
    const rawValue = config.getValue(adaptedStats, props.rangeMinutes)
