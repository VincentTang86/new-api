...
export function ModelMutateDrawer({
  ...
  // Pricing is not a model field: it lives in system options, which only a
  // super admin may read or write. Admins still maintain model metadata here,
  // so hide the pricing block for them rather than offering inputs whose
  // values would be silently dropped on save.
  const canManagePricing = useAuthStore(
