...
import { BILLING_PRICING_VARS } from '../lib/billing-expr'
import {
  formatTimeWindows,
...
import {
  getRateConditions,
  TIER_FIELD_REFERENCE_LANES,
  type RateCondition,
} from '../lib/rate-conditions'
import type {
  PriceType,
  PricingModel,
  ReferencePrice,
...
export function ModelDetailsPricingTable(props: {
  ...
  // One row per rate condition (tier × time variant), from the shared module
  // so the settings card derives the exact same list and keys.
  const rateRows = useMemo(() => getRateConditions(model, t), [model, t])
  ...
  const referenceRows: { key: string; label: string; lanes: ReferencePrice }[] =
    [
      {
        key: 'official',
    ...
    ].flatMap((row) => (row.lanes ? [{ ...row, lanes: row.lanes }] : []))
  ...
  const cellPrice = (
    ...
    if (column.tierField) {
      const value = Number(row.tier[column.tierField])
      if (!Number.isFinite(value) || value <= 0) {
        return LANDING_PRICE_PLACEHOLDER
      }
      const multiplier = row.variant?.multiplier ?? 1
      return formatLandingPrice((value * multiplier * ratio) / divisor)
    }
  ...
  // The generic unconditioned row is stated by the default (flat) reference
  // price; every real condition needs its own configured price — an absent
  // one renders as dashes rather than borrowing the default, so a peak-hour
  // row never displays a rate the source does not actually list for it.
  const referenceLanes = (
    price: ReferencePrice,
    cond: RateCondition
  ): ReferencePriceLanes | undefined =>
    cond.key === '' ? price : price.by_condition?.[cond.key]
      ...
      <table className='w-full min-w-max border-collapse text-left'>
        ...
        <tbody>
          {groups.map((group) => {
            ...
            return rateRows.map((row, rowIndex) => (
              <tr
                key={`${group}-${row.key || 'flat'}`}
                className={
                  rowIndex === 0
          ...
          {isTokenBased &&
            referenceRows.map((row) =>
              // The reference block mirrors the gateway's rate-condition rows
              // so the comparison reads line by line. Each condition states
              // the price the source lists for it (resolved by key); the
              // generic row falls back to the source's default price.
              rateRows.map((rateRow, rowIndex) => (
                <tr
                  key={`${row.key}-${rateRow.key || 'flat'}`}
                  className={
                    rowIndex === 0
                ...
                >
                  ...
                  {columns.map((column) => (
                    ...
                    >
                      {formatLandingPrice(
                        referencePrice(
                          referenceLanes(row.lanes, rateRow),
                          column.referenceLane
                        ) / divisor
                      )}
                    </td>
                  ))}
...
export function ModelDetailsPricingNotes(props: {
  ...
  const windowVariants = getTimeRateVariants(props.model).filter(
    (variant) => variant.windows.length > 0
  )
  ...
  return (
    <div className='space-y-0.5 text-[11px] leading-relaxed text-(--pd-faint)'>
      ...
      {windowVariants.map((variant) => (
        <p key={`${variant.labelKey}-${variant.multiplier}`}>
          {timeRateVariantLabel(variant, t)}:{' '}
          {formatTimeWindows(variant.windows, t)}
        </p>
      ))}
      {isTimeAware && (
        <p>
