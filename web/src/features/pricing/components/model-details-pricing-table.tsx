...
const GENERIC_TIER_LABELS = new Set(['base', 'default'])

/** One rendered rate row: a parsed tier crossed with a time-rate variant. */
interface RateRow {
  tier: ParsedTier
...
export function ModelDetailsPricingTable(props: {
  ...
  const timeVariants = useMemo(() => getTimeRateVariants(model), [model])
  ...
  // Time-conditional multipliers become their own rows per tier (the design's
  // Peak Hours / Off-Peak Hours sub-rows); without them each tier is one row.
  ...
  const cellPrice = (column: PriceColumn, row: RateRow, ratio: number) => {
    if (column.tierField) {
  ...
  const rateConditionText = (row: RateRow) => {
    const raw = (row.tier.label || '').trim()
    const tierLabel =
      raw && !GENERIC_TIER_LABELS.has(raw.toLowerCase()) ? raw : t('Standard')
    if (!row.variant) return tierLabel
    const variantLabel = timeRateVariantLabel(row.variant, t)
    return baseTiers.length > 1
      ? `${tierLabel} · ${variantLabel}`
      : variantLabel
  }
        ...
        <tbody>
          {groups.map((group) => {
            ...
            return rateRows.map((row, rowIndex) => (
              <tr
                key={`${group}-${row.tier.label || rowIndex}-${row.variant?.multiplier ?? 'flat'}`}
                className={
                  rowIndex === 0
