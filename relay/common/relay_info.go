...
type RelayInfo struct {
	...
	// EstimatedUpstreamTokens 是适配器按请求规格预估的上游 token 数，0 表示估不出。
	// 异步任务提交时上游还没回 usage，这个值既决定押金，也写进提交日志的 token 列，
	// 否则一笔视频消费在日志里是零 token；实际用量由结算日志记录。
	EstimatedUpstreamTokens int
