...
// other log types and other users never leak into the numbers.
func TestGetUserLogMetrics(t *testing.T) {
	require.NoError(t, DB.Exec("DELETE FROM logs").Error)

	logs := []Log{
		// user 1, in range: three consume logs
		{UserId: 1, Type: LogTypeConsume, CreatedAt: 1000, PromptTokens: 100, CompletionTokens: 50, UseTime: 2},
		{UserId: 1, Type: LogTypeConsume, CreatedAt: 1100, PromptTokens: 200, CompletionTokens: 100, UseTime: 4},
		{UserId: 1, Type: LogTypeConsume, CreatedAt: 1200, PromptTokens: 300, CompletionTokens: 150, UseTime: 6},
		// user 1, in range: one error log (tokens/use_time must not affect consume averages)
		{UserId: 1, Type: LogTypeError, CreatedAt: 1150, UseTime: 30},
		...
		{UserId: 1, Type: LogTypeConsume, CreatedAt: 2000, PromptTokens: 999, CompletionTokens: 999, UseTime: 99},
		{UserId: 1, Type: LogTypeError, CreatedAt: 2000},
		// user 2, in range: must be excluded by user scoping
		{UserId: 2, Type: LogTypeConsume, CreatedAt: 1100, PromptTokens: 777, CompletionTokens: 777, UseTime: 7},
